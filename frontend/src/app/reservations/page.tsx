...
export default function Reservations() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult({
        success: false,
        message: error.response?.data?.error || '予約処理中にエラーが発生しました。'
      })
    } finally {
      setIsLoading(false)
    }
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
            <form onSubmit={handleSubmit}>
              <div className="mb-6">
                <label htmlFor="reservation" className="block text-sm font-medium text-gray-700 mb-2">
                  予約内容（自然言語で入力してください）
                </label>
                <textarea
                  id="reservation"
              ...
              {result.success && result.details && (
                <div className="bg-white rounded-md p-4">
                  ...
                  <div className="text-sm text-gray-600 space-y-1">
                    <p><strong>解釈された内容:</strong> {result.details.parsedRequest}</p>
                    <p><strong>予定時間:</strong> {result.details.estimatedTime}</p>
                    <p><strong>GPU種類:</strong> {result.details.gpuType}</p>
                    <p><strong>台数:</strong> {result.details.quantity}台</p>
                    <p><strong>AI優先度:</strong> {result.details.priority}/100</p>
                    <p><strong>判定理由:</strong> {result.details.reasoning}</p>
